...
def accept_request(request, friend_id) -> Friend:
    ...
    if status != Status.PENDING:
        logger.warning(
            f"Friendship with ID {friendship_id} is not in pending state, current state: {status}"
        )
    ...
    if receiver_id != current_user_id:
        logger.warning(
            f"User {current_user_id} is not the receiver of the friend request"
        )
    ...
    logger.info(
        f"Friend request from {sender_id} to {current_user_id} accepted successfully"
    )
