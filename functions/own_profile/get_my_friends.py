...
def get_my_friends(request) -> FriendsResponse:
    ...
    try:
        # Use a single efficient query with array-contains and in operator for multiple statuses
        friendships_query = db.collection(Collections.FRIENDSHIPS) \
            .where(FriendshipFields.MEMBERS, "array-contains", current_user_id) \
            .where(FriendshipFields.STATUS, "in", [Status.ACCEPTED, Status.PENDING])
