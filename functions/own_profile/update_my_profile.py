...
def update_profile(request):
    ...
    username_changed = (
        ...
        and profile_data_input.username != current_profile_data.get(ProfileFields.USERNAME)
    )
    ...
    if hasattr(profile_data_input, "username") and profile_data_input.username is not None:
    ...
    if hasattr(profile_data_input, "location") and profile_data_input.location is not None:
        profile_updates[ProfileFields.LOCATION] = profile_data_input.location
    
    if hasattr(profile_data_input, "birthday") and profile_data_input.birthday is not None:
        profile_updates[ProfileFields.BIRTHDAY] = profile_data_input.birthday
    
    if hasattr(profile_data_input, "notification_settings") and profile_data_input.notification_settings is not None:
        profile_updates[ProfileFields.NOTIFICATION_SETTINGS] = profile_data_input.notification_settings
    ...
    if username_changed or name_changed or avatar_changed:
        logger.info(f"Updating username/name/avatar references for user {current_user_id}")
        ...
        for invitation_doc in invitation_docs:
            ...
            if username_changed:
                invitation_updates[InvitationFields.USERNAME] = profile_updates[ProfileFields.USERNAME]
            
            if name_changed:
                invitation_updates[InvitationFields.NAME] = profile_updates[ProfileFields.NAME]
            
            if avatar_changed:
                invitation_updates[InvitationFields.AVATAR] = profile_updates[ProfileFields.AVATAR]
        ...
        for friendship_doc in friendship_sender_docs:
            ...
            if username_changed:
                friendship_updates[FriendshipFields.SENDER_USERNAME] = profile_updates[ProfileFields.USERNAME]
            
            if name_changed:
                friendship_updates[FriendshipFields.SENDER_NAME] = profile_updates[ProfileFields.NAME]
            
            if avatar_changed:
                friendship_updates[FriendshipFields.SENDER_AVATAR] = profile_updates[ProfileFields.AVATAR]
        ...
        for friendship_doc in friendship_receiver_docs:
            ...
            if username_changed:
                friendship_updates[FriendshipFields.RECEIVER_USERNAME] = profile_updates[ProfileFields.USERNAME]
            
            if name_changed:
                friendship_updates[FriendshipFields.RECEIVER_NAME] = profile_updates[ProfileFields.NAME]
            
            if avatar_changed:
                friendship_updates[FriendshipFields.RECEIVER_AVATAR] = profile_updates[ProfileFields.AVATAR]
            ...
            for i, member_profile in enumerate(member_profiles):
                if member_profile.get(ProfileFields.USER_ID) == current_user_id:
                    ...
                    if username_changed:
                        batch.update(
                            group_ref, 
                            {f"{GroupFields.MEMBER_PROFILES}.{i}.{ProfileFields.USERNAME}": profile_updates[ProfileFields.USERNAME]}
                        )
                    
                    if name_changed:
                        batch.update(
                            group_ref, 
                            {f"{GroupFields.MEMBER_PROFILES}.{i}.{ProfileFields.NAME}": profile_updates[ProfileFields.NAME]}
                        )
                    
                    if avatar_changed:
                        batch.update(
                            group_ref, 
                            {f"{GroupFields.MEMBER_PROFILES}.{i}.{ProfileFields.AVATAR}": profile_updates[ProfileFields.AVATAR]}
                        )
