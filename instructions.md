...
- **Pagination**
  - For paginated queries, use the `startAfter` method:
    ...
    const next_cursor =
      last_timestamp && results.docs.length === limit ? last_timestamp : null;
    ```
...
- **Pagination Timestamps**
  - Use `formatTimestamp` for pagination tokens:
    ...
    const next_cursor =
      last_timestamp && updates.length === limit
...
- **Exception Handling**
  ...
  - Wrap function implementations in try-catch blocks:
    ...
    try {
      ...
      logger.error(
        `Error in function: ${error instanceof Error ? error.message : "Unknown error"}`,
        { error },
      );
...
- **Log Levels**
  ...
  - `error`: Errors that cause function failure (exceptions, critical missing data)
    ```typescript
    logger.error(
      `Error retrieving profile for user ${userId}: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
    ```
  - `debug`: Detailed information for debugging purposes
    ```typescript
    logger.debug(
      `Processing request with parameters: ${JSON.stringify(req.query)}`,
    );
    ```
- **Contextual Information**
  - Include relevant context in log messages:
    ...
    logger.warn(`Profile not found for user: ${userId}`);
    logger.error(
      `Error retrieving data: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
    ```
  ...
  - For pagination, include pagination parameters:
    ```typescript
    logger.info(
      `Pagination parameters - limit: ${limit}, after_cursor: ${after_cursor}`,
    );
    ```
  ...
  - Follow a consistent structure for all functions:
    ...
    export const myFunction = async (req: Request, res: Response) => {
      ...
      try {
        ...
        logger.error(
          `Error in function for user ${req.userId}: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        return res.status(500).json({
          code: 500,
          name: "Internal Server Error",
...
- **Pagination Implementation**
  ...
  - For paginated endpoints, use consistent pagination parameters and logic:
    ...
    // Set up pagination for the next request
    const next_cursor =
      last_timestamp && updates.length === limit ? last_timestamp : null;
...
- **Friend Relationship Handling**
  - For functions that deal with friend relationships:
    ...
    const friend_ref = user_profile_ref
    ...
    if (!is_friend) {
      logger.warn(
        `User ${req.userId} attempted to view profile of non-friend ${friendId}`,
      );
